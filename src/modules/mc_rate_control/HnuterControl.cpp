...
{
	// Hnuter can rotate the front thrust vector through 180 degrees. Beyond a
	// 90-degree body attitude the body-Z component required for world-frame
	// hover changes sign, even though every front motor remains unidirectional.
	return math::constrain(force / math::max(max_force, 1.f), -1.f, 1.f);
}
...
float HnuterControl::synchronizedAttitudeRate(float requested_rate_deg_s, float actuator_rate_deg_s,
		float actuator_tau_s, float actuator_delay_s, float allowed_error_deg)
{
	...
	const float lag_s = math::max(actuator_tau_s, 0.f) + math::max(actuator_delay_s, 0.f);

	if (lag_s > 1e-3f) {
		rate_deg_s = math::min(rate_deg_s, math::max(allowed_error_deg, 0.5f) / lag_s);
	}

	return math::radians(rate_deg_s);
}

Vector3f HnuterControl::continuousEulerFromRotation(const Dcmf &rotation, const Vector3f &reference)
{
	const Eulerf principal_euler{rotation};
	Vector3f principal{
		matrix::unwrap_pi(reference(0), principal_euler.phi()),
		matrix::unwrap_pi(reference(1), principal_euler.theta()),
		matrix::unwrap_pi(reference(2), principal_euler.psi())
	};

	// The same rotation also has the representation
	// (roll + pi, sign(pitch) * pi - pitch, yaw + pi). Select the
	// representation nearest the previous command so crossing +/-90 degrees
	// cannot fold a 110-degree pitch command back to 70 degrees.
	const float equivalent_pitch = principal_euler.theta() >= 0.f
				       ? M_PI_F - principal_euler.theta()
				       : -M_PI_F - principal_euler.theta();
	Vector3f equivalent{
		matrix::unwrap_pi(reference(0), principal_euler.phi() + M_PI_F),
		matrix::unwrap_pi(reference(1), equivalent_pitch),
		matrix::unwrap_pi(reference(2), principal_euler.psi() + M_PI_F)
	};

	const Vector3f principal_error = principal - reference;
	const Vector3f equivalent_error = equivalent - reference;
	return equivalent_error.norm_squared() < principal_error.norm_squared() ? equivalent : principal;
}
			   ...
			   const matrix::Vector3f &rates, Output &output)
{
	...
	Vector3f allocation_accel_residual{};

	if (allocator_status_valid && !allocator_status.thrust_setpoint_achieved) {
	...
	const bool on_ground = landed || maybe_landed;
	...
	const float horizontal_authority = on_ground ? 0.f
					   : (takeoff_tilt_suppress_time_s > FLT_EPSILON
					      ? math::constrain(time_since_takeoff_s / takeoff_tilt_suppress_time_s, 0.f, 1.f) : 1.f);
	const bool xy_lock_active = hnuter_translation_control_active && !on_ground
				    && time_since_takeoff_s < takeoff_xy_lock_time_s;
	...
	if (landed || maybe_landed) {
		...
		const Vector3f saturation_residual = acc_unsaturated - acc_des + allocation_accel_residual;

		for (int i = 0; i < 3; i++) {
			const bool drives_further_into_saturation = saturation_residual(i) * vel_error(i) > 0.f;

			const bool authority_available = i == 2 || horizontal_authority >= 0.999f;

			if (velocity_sp_valid[i] && authority_available && !drives_further_into_saturation) {
	...
	if (manual_attitude_altitude_mode || on_ground) {
		f_body(0) = 0.f;
		f_body(1) = 0.f;

	} else if (horizontal_authority < 1.f) {
		f_body(0) *= horizontal_authority;
		f_body(1) *= horizontal_authority;
	}

	const float takeoff_ramp_tilt_limit = takeoff_tilt_limit_rad
					      + horizontal_authority * (xy_lock_tilt_limit_rad - takeoff_tilt_limit_rad);
	const float tilt_limit = on_ground ? takeoff_tilt_limit_rad :
				 (xy_lock_active ? takeoff_ramp_tilt_limit : default_tilt_limit_rad);
	...
	Vector2f commanded_attitude_rp{};
	bool commanded_attitude_rp_valid = false;
	bool rc_attitude_mode_enabled = false;
	...
	if (!r_des_valid) {
		...
		if (rc_attitude_mode_enabled) {
			if (!_rc_attitude_initialized) {
				...
				_rc_level_switch_previous = manual_sp_fresh && PX4_ISFINITE(manual_sp.aux3)
							    && manual_sp.aux3 > 0.5f;
				_rc_roll_input_previous = false;
				_rc_pitch_input_previous = false;
				_rc_yaw_input_previous = false;
			...
			const float roll_input = manual_sp_fresh && PX4_ISFINITE(manual_sp.aux1)
						 ? applyDeadband(manual_sp.aux1, deadband) : 0.f;
			const float pitch_input = manual_sp_fresh && PX4_ISFINITE(manual_sp.aux2)
						  ? applyDeadband(manual_sp.aux2, deadband) : 0.f;
			const float yaw_input = manual_sp_fresh && PX4_ISFINITE(manual_sp.yaw)
						? applyDeadband(manual_sp.yaw, deadband) : 0.f;
			const bool level_switch = manual_sp_fresh && PX4_ISFINITE(manual_sp.aux3) && manual_sp.aux3 > 0.5f;
			...
			const Vector3f continuous_current = continuousEulerFromRotation(R,
							  Vector3f{_rc_attitude_sp(0), _rc_attitude_sp(1), _rc_yaw_sp});
			...
			if (manual_sp_fresh && !roll_input_active && _rc_roll_input_previous) {
				_rc_attitude_sp(0) = continuous_current(0);
			}

			if (manual_sp_fresh && !pitch_input_active && _rc_pitch_input_previous) {
				_rc_attitude_sp(1) = continuous_current(1);
			}

			if (manual_sp_fresh && !yaw_input_active && _rc_yaw_input_previous) {
				_rc_yaw_sp = atan2f(sinf(continuous_current(2)), cosf(continuous_current(2)));
			}

			const bool synchronize_to_tilt_dynamics = _param_hntr_tdyn_en.get();
			const auto attitude_rate_limit = [synchronize_to_tilt_dynamics](float requested_rate_deg_s,
							 float actuator_rate_deg_s, float actuator_tau_s,
							 float actuator_delay_s, float allowed_error_deg) {
				return synchronize_to_tilt_dynamics
				       ? HnuterControl::synchronizedAttitudeRate(requested_rate_deg_s, actuator_rate_deg_s,
						       actuator_tau_s, actuator_delay_s, allowed_error_deg)
				       : math::radians(math::max(requested_rate_deg_s, 0.f));
			};

			if (_rc_level_return_active) {
				const float roll_level_rate = attitude_rate_limit(_param_hntr_rc_lvl_r.get(),
							      _param_hntr_t2_rate.get(), _param_hntr_t2_tau.get(), _param_hntr_t2_dly.get(),
							      _param_hntr_sync_err.get());
				const float pitch_level_rate = attitude_rate_limit(_param_hntr_rc_lvl_r.get(),
							       _param_hntr_t1_rate.get(), _param_hntr_t1_tau.get(), _param_hntr_t1_dly.get(),
							       _param_hntr_sync_err.get());
				_rc_attitude_sp(0) = slewToZero(_rc_attitude_sp(0), roll_level_rate, dt, rc_attitude_rate_sp(0));
				_rc_attitude_sp(1) = slewToZero(_rc_attitude_sp(1), pitch_level_rate, dt, rc_attitude_rate_sp(1));
				...
				const float roll_rate_limit = attitude_rate_limit(_param_hntr_rc_rate_r.get(),
							      _param_hntr_t2_rate.get(), _param_hntr_t2_tau.get(), _param_hntr_t2_dly.get(),
							      _param_hntr_sync_err.get());
				const float pitch_rate_limit = attitude_rate_limit(_param_hntr_rc_rate_p.get(),
							       _param_hntr_t1_rate.get(), _param_hntr_t1_tau.get(), _param_hntr_t1_dly.get(),
							       _param_hntr_sync_err.get());
				rc_attitude_rate_sp(0) = roll_input_active
							 ? roll_input * roll_rate_limit : 0.f;
				rc_attitude_rate_sp(1) = pitch_input_active
							 ? pitch_input * pitch_rate_limit : 0.f;
		...
		r_des_valid = true;
		commanded_attitude_rp = attitude_sp_rp;
		commanded_attitude_rp_valid = true;

		if (!rc_attitude_mode_enabled && have_traj && PX4_ISFINITE(traj_sp.yawspeed)) {
	...
	// Preserve the commanded Euler branch for the feed-forward rate mapping.
	// Reconstructing it from R_des would fold pitch back into +/-90 degrees.
	const float roll_des = commanded_attitude_rp_valid ? commanded_attitude_rp(0) : euler_des.phi();
	const float pitch_des = commanded_attitude_rp_valid ? commanded_attitude_rp(1) : euler_des.theta();
	...
	if (landed || maybe_landed) {
		...
		for (int i = 0; i < 3; i++) {
			...
			const float unallocated_physical_torque = i == 1 ? -allocator_status.unallocated_torque[i]
	...
	if (use_rates_sp && !hnuter_translation_control_active && !manual_attitude_altitude_mode) {
		...
		if (allocator_status_valid && !allocator_status.torque_setpoint_achieved) {
			for (int i = 0; i < 3; i++) {
				const float unallocated_physical_torque = i == 1 ? -allocator_status.unallocated_torque[i]
						: allocator_status.unallocated_torque[i];
