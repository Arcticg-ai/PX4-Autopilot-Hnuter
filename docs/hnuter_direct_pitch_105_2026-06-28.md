...
2026-07-03 起，Servo 3/4 和 `CA_SV_TL2/3` 进一步扩展为
`-180..180 deg`，用于下半球侧面轨迹。
...
`-3.229..+3.229 rad`，Servo 2/3 为 `-3.142..+3.142 rad`。
...
- 普通模式保留二级 `theta_limit_deg=45`；球面轨迹 3 单独允许 `180 deg`。
- 二级输出映射改为 `theta / 180 deg`。逆解在两组等价万向节角之间选择
  离上一帧最近的分支，使第二级可连续穿过 `90 deg`，避免一级角突跳
  `180 deg`。
