...
| `HNTR_RC_ANG_MAX` | 累计姿态绝对值限制 | 90° | 45° |
| `HNTR_RC_LVL_R` | 渐进回平最大速度 | 20 deg/s | 15 deg/s |

`HNTR_RC_ANG_MAX` 的参数范围允许到 180°，但实机首次测试应保持 45° 或更低。只有在
姿态环、控制分配和线束活动范围均验证后才能逐步放大。
...
该残余不影响通道状态机和渐进回平功能判定，但不应直接在实机尝试 45°。首次测试应将
`HNTR_RC_ANG_MAX` 设为 10–15°，在测试架确认 yaw/roll/pitch 响应后再逐步放大。
