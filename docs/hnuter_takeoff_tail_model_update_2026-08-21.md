...
实机默认 `HNTR_ACC_XY` 从 `3.0` 调整为 `8.0 m/s²`。对 `4.5 kg` 机体，对应理论水平命令包络约 `36 N`。该值只是指令限幅，不等于已测得接触力；实际接触力仍受电机、倾转角、舵机速率、姿态力矩和结构约束影响。
...
5. 最后再逐级进行接触测试；首次不直接使用 36 N 满指令。
