...
## 23. 2026-08-08 3131ddd4 实机大 Pitch 失稳记录

- 版本化保存 `log_130`/`log_131` 对应的固件身份、原始文件 SHA-256、筛选后的
  实飞关键参数和量化分析；公开分支不包含带系统 UUID/位置的原始 ULog。
- 两次实飞最大 Pitch 为约 `72.3/74.2 deg`，但实际手动 Position Pitch 目标受
  `HNTR_RC_ANG_MAX=45 deg` 限制，记录角度属于失控过冲而非 90 deg 正常跟踪。
- 失稳窗口中 Pitch 反馈力矩约 `41%` 时间触及 `HNTR_TAU_P=10 Nm`，尾推指令
  分别过零 10/38 次；实际保存的 Pitch 参数为 `20/8/10`、积分 `0.06`、偏置
  `0.09`，尾电机和四路舵机 slew 参数均为 0。
- 报告将 Pitch 偏置越过名义力矩上限、尾推缺少动态/换向保护、激进参数及执行器
  滞后列为主要风险；一级倾转共同摆动符合分配耦合，不等同于单舵机故障。
- 本固件和该参数组合标记为不可继续大姿态飞行；完整报告见
  `docs/flight_tests/2026-08-07_3131ddd4_pitch_instability/README.md`。

## 24. 2026-08-08 Pitch 与尾电机安全修改
...
## 26. 2026-08-08 大姿态目标跟踪、动态补偿与起飞平滑

- 将手动 Position 的姿态目标由欧拉角积分改为完整四元数积分；松杆继续锁存实测
  姿态，Pitch 不增加绝对 90 度限制。
- 新增相对实测姿态的软/硬误差治理器，并用 SO(3) 对数映射替换会在 180 度失效的
  正弦姿态误差；目标角速度不再经过欧拉角反算。
- 用 `HNTR_CG_X/Z` 和当前指令力生成动态 Pitch 前馈，条件迁移日志 132 的固定
  `PITCH_BIAS=0.09`，并启用很小且有限幅的 Pitch 积分起始值。
- 新增起飞锁定退出渐变，同时迁移日志 132 保存的激进 Position 参数组合。
- 新增 `hnuter_control_status` 日志、命令相对姿态故障判据，以及考虑换向保护、推力、
  关节和舵机指令速率限制的模型分配残差。
- 详细证据、参数、限制和验证顺序见
  `docs/hnuter_full_attitude_followup_2026-08-08.md`。

## 27. log 133 解锁倾覆纠正（2026-08-08）
...
## 28. log 134 尾部下垂与 Pitch 重力配平修正（2026-08-09）

- `log_134` 证明尾电机未进入换向等待或输出饱和：稳定飞行阶段 Motor5 指令约
  `0.300`、PWM 约 `1615 us`，但 Pitch 从初始化时约 `1.3 deg` 留下约
  `8 deg` 的稳态误差。`log_133` 的过大尾推同时证明执行器具备足够控制能力。
- 保留 Motor5 只响应 Pitch 力矩的分配原则，不恢复会跟随总推力的
  `HNTR_TAIL_COMP`。
- 在姿态控制器中用当前姿态、`HNTR_MASS` 和 `HNTR_CG_X/Z` 计算重力在机体系的
  Pitch 力矩前馈；该项不随油门变化，并在接近垂直 Pitch 时自然减小为 CG-Z
  分量。
- 固定 `HNTR_PITCH_BIAS` 从 `0.09` 降为剩余模型误差配平 `0.02`；Pitch 积分改为
  `I_P=0.01`、`ILIM_P=0.5 Nm`。`KR_P=5`、`D_P=2.5` 和最终
  `TAU_P=8 Nm` 保持不变。
- 撤销未经用户要求的弱位置环默认值，恢复 `log_132` 实飞组合
  `3.75/9.05/0.20/3.0`（XY）和 `3.5/4.0/45.6`（Z）；保留
  `HNTR_TO_RAMP_T=4 s`、释放渐变和渐变开始时一次清除 XY 积分状态。
- `hnuter_control_status` 改为非可选默认日志主题，并新增重力前馈、剩余偏置两个
  独立字段，避免 Logger 启动早于首次发布时把该主题永久排除。
- Gazebo 模型显式设置 `HNTR_CG_X/Z=0`，因为仿真器在模型重心处施加重力；实机
  重力配平不能直接重复叠加到该模型。
- 无参数值迁移逻辑；已有飞控烧录后必须手动写入并保存本版参数表。
- 详细分析、公式、参数和验证边界见
  `docs/hnuter_log134_pitch_gravity_correction_2026-08-09.md`。

## 29. 姿态解耦、动态尾电机保护与残差闭环（2026-08-13）

- 按调试清单第 `9、13、14、15、16、17、18、21` 项实施修改。
- 尾电机换向保护升级为按幅值变化的卸载、等待和反向建立状态机；新增独立动态参考
  推力及增推、减推、换向后斜率参数，避免用尚未辨识的 `HNTR_MAX_TAIL_T` 判断
  实机换向强度。
- 分配器根据尾推保护、主推力限幅、倾转限位和舵机速率限制重建模型可实现六维力与
  力矩；Pitch 姿态积分和位置速度积分使用该残差做抗饱和。
- 手动姿态目标改为世界航向加二维倾转表示，AUX1、AUX2 和 Yaw 分开更新；松杆只
  锁存对应轴，避免把偏航误差和其他姿态轴写回目标。
- 尾电机受限时保留 Pitch P/D，冻结 Pitch I，并阻止继续扩大误差的 Pitch 目标
  增量；实机固定 `HNTR_PITCH_BIAS` 默认值改为 `0`。
- 新增 `hnuter_allocator_status` 和扩展 `hnuter_control_status`，记录受限前后尾推、
  PWM/RPM 估算、状态机、饱和、残差与抗积分状态。
- 实机与 Gazebo 继续使用独立 airframe、执行器映射、齿轮/重心和尾电机动态参数；
  `HNTR_MAX_TAIL_T=85.48 N` 和正反推对称静态模型本次未修改，等待推力台架标定。
- SITL 编译、HNUTER Gazebo 启动及 CUAV 7-Nano 固件编译通过；固件 Flash 为
  `1880044 / 1966080 bytes (95.62%)`。
- 完整算法、参数表、日志字段、未修改边界和安全验证要求见
  `docs/hnuter_attitude_tail_residual_update_2026-08-13.md`。

## 30. 2026-07-30 `144bd9fe` OK 版本（实机验证通过基线）
