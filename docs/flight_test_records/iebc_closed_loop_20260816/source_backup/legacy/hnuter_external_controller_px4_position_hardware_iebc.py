...
This revision adds an optional reference-level Interaction-Energy Barrier Control
(IEBC) layer. IEBC is disabled by default and must be explicitly configured for
experiments.
"""
...
class InteractionEnergyBarrierFilter:
    """
    Reference-level Interaction-Energy Barrier Control (IEBC).

    This is the translational interaction-subspace specialization used by the
    current PX4 position-offboard node.  The paper-level construction is:

        P_I_hat  <- dynamics residual / mechanical power balance
        S_e_bar  <- conservative environment-storage upper bound
        E_I      = 0.5 * lambda_bar * v_I^2 + S_e_bar
        h_I      = E_max - E_I

    A one-constraint CBF-QP is solved analytically along the configured
    interaction axis.  A sampled-data kinetic-energy guard is applied as a
    numerical safety backstop when |v_I| is close to zero.  Orthogonal
    translational references pass through unchanged.

    IMPORTANT: the theory assumes an estimate of the ACTUAL actuator wrench.
    This node therefore supports an external world-frame actuator-force topic.
    The built-in ``proxy`` source is only for software/Gazebo bring-up and uses
    the previously issued acceleration reference as an approximate force.
    """
    ...
    def __init__(self, logger=None):
        ...
        self.discrete_eta = float(np.clip(
            env_float('HNUTER_IEBC_DISCRETE_ETA', 1.0), 1e-3, 1.0))
        self.power_lpf_tau = max(env_float('HNUTER_IEBC_POWER_LPF_TAU_S', 0.05), 0.0)
        ...
        self.engage_power_w = max(env_float('HNUTER_IEBC_ENGAGE_POWER_W', 0.20), 0.0)
        self.always_active = env_bool('HNUTER_IEBC_ALWAYS_ACTIVE', False)
        self.velocity_epsilon = max(env_float('HNUTER_IEBC_VELOCITY_EPS_MPS', 0.02), 1e-4)
        self.max_interaction_accel = max(env_float('HNUTER_IEBC_MAX_ACCEL_MPS2', 2.0), 0.05)
        self.ref_kp = max(env_float('HNUTER_IEBC_REF_KP', 3.0), 0.0)
        self.ref_kd = max(env_float('HNUTER_IEBC_REF_KD', 2.5), 0.0)
        ...
        if self.enabled and not self.valid_configuration:
            self._warn(
                'IEBC 已请求启用，但 HNUTER_IEBC_MASS_KG、HNUTER_IEBC_LAMBDA_BAR_KG '
                '和 HNUTER_IEBC_E_MAX_J 必须为正值；当前 IEBC 将旁路。')
        if self.enabled and self.valid_configuration and self.wrench_source == 'proxy':
            self._warn(
                'IEBC 当前使用 actuator-wrench proxy，仅适合 Gazebo/软件联调。论文实验应通过 '
                'HNUTER_IEBC_WRENCH_SOURCE=external 输入实际执行器 wrench 重构结果。')
    ...
    def reset(self) -> None:
        self.interaction_active = bool(self.always_active)
        self.h_prev = None
        self.power_hat_raw = 0.0
        self.power_hat = 0.0
        self.work_hat = 0.0
        self.delta_work = 0.0
        self.storage_bound = 0.0
        self.safe_s = None
        self.safe_v = None
        self.debug = {
    ...
    def _mechanical_energy(self, position_enu: np.ndarray, velocity_enu: np.ndarray) -> float:
        # Translational specialization of H_r = T + U_g.  A constant shift in z
        # does not affect the work balance, so PX4 local ENU altitude is valid.
        return float(
            0.5 * self.mass * np.dot(velocity_enu, velocity_enu)
            + self.mass * self.g * position_enu[2]
        )

    def _update_interaction_work(
        ...
        P_a = float(np.dot(actuator_force_enu, velocity_enu))
        ...
        # P_I_hat = P_a - dH_r/dt. Positive means mechanical power delivered
        # from the robot to the environment under the paper sign convention.
        self.power_hat_raw = P_a - dH_dt
        ...
        if not self.interaction_active:
            if self.always_active or self.power_hat >= self.engage_power_w:
                self.interaction_active = True
                # Define the contact/interaction onset t_c here.
                self.work_hat = 0.0
                self.delta_work = 0.0
                self.storage_bound = 0.0
            else:
        ...
        self.work_hat += self.power_hat * dt
        uncertainty_power = (
            self.power_margin_w
            + self.force_error_bound_n * float(np.linalg.norm(velocity_enu))
        )
        self.delta_work += uncertainty_power * dt
        self.storage_bound = max(0.0, self.work_hat + self.delta_work)
    ...
    def filter_reference(
        ...
        # Fail-open: if IEBC is disabled/misconfigured or actual wrench input is
        # required but unavailable, pass the nominal reference unchanged.
        ...
        a_nom_ff = float(np.dot(self.axis, nominal_acceleration_enu))
        ...
        # When interaction has not started, IEBC is exactly transparent and the
        # internal safe reference is synchronized to the nominal trajectory.
        if not self.interaction_active:
            self.safe_s = s_nom
            self.safe_v = v_nom
            self.debug.update({
                'enabled': True, 'active': False, 'p_hat': self.power_hat,
                'w_hat': self.work_hat, 's_bar': self.storage_bound,
                'k_i': kinetic_i, 'e_i': energy_i, 'h_i': h_i, 'v_i': v_i,
                'a_nom_i': a_nom_ff, 'a_safe_i': a_nom_ff, 'v_limit': math.inf,
            })
            return (nominal_position_enu.copy(), nominal_velocity_enu.copy(),
                    nominal_acceleration_enu.copy())

        # Reference-tracking acceleration. This is the nominal objective of the
        # reference-level QP; the barrier modifies it only when needed.
        a_nom_i = (
            a_nom_ff
            + self.ref_kp * (s_nom - self.safe_s)
            + self.ref_kd * (v_nom - self.safe_v)
        )

        # Continuous-time IEBC constraint:
        #   lambda_bar * v_I * a_I + dot(S_bar) <= gamma * h_I
        # which is a single affine inequality in a_I and therefore admits the
        # analytic projection below (the 1-D solution of the CBF-QP).
        storage_rate_bound = (
            max(self.power_hat, 0.0)
            + self.power_margin_w
            + self.force_error_bound_n * float(np.linalg.norm(measured_velocity_enu))
        )
        coeff = self.lambda_bar * v_i
        rhs = self.gamma * h_i - storage_rate_bound
        a_safe_i = a_nom_i
        if abs(coeff) > self.lambda_bar * self.velocity_epsilon:
            if coeff * a_safe_i > rhs:
                a_safe_i = rhs / coeff

        # Sampled-data energy guard. It also removes the relative-degree
        # degeneracy of the continuous CBF at v_I ~= 0.
        h_positive = max(h_i, 0.0)
        e_allowed_next = self.e_max - (1.0 - self.discrete_eta) * h_positive
        s_bar_next = self.storage_bound + storage_rate_bound * dt
        k_available = max(0.0, e_allowed_next - s_bar_next)
        v_limit = math.sqrt(max(0.0, 2.0 * k_available / self.lambda_bar))
        a_min = (-v_limit - self.safe_v) / max(dt, 1e-6)
        a_max = (v_limit - self.safe_v) / max(dt, 1e-6)
        a_safe_i = float(np.clip(a_safe_i, a_min, a_max))
        a_safe_i = float(np.clip(
            a_safe_i, -self.max_interaction_accel, self.max_interaction_accel))

        # Integrate only the interaction-axis safe reference. Orthogonal
        # components remain exactly equal to the nominal reference.
        self.safe_v += a_safe_i * dt
        self.safe_s += self.safe_v * dt

        safe_position = nominal_position_enu + self.axis * (self.safe_s - s_nom)
        safe_velocity = nominal_velocity_enu + self.axis * (self.safe_v - v_nom)
        safe_acceleration = nominal_acceleration_enu + self.axis * (a_safe_i - a_nom_ff)

        self.debug.update({
            'enabled': True, 'active': True, 'p_hat': self.power_hat,
            'w_hat': self.work_hat, 's_bar': self.storage_bound,
            'k_i': kinetic_i, 'e_i': energy_i, 'h_i': h_i, 'v_i': v_i,
            'a_nom_i': a_nom_i, 'a_safe_i': a_safe_i, 'v_limit': v_limit,
        })
...
class HnuterController(Node):
    ...
    def __init__(self):
        ...
        # IEBC: reference-level interaction-energy safety filter.  The existing
    ...
    def _iebc_actuator_force_estimate(self):
        ...
        # Software/Gazebo bring-up proxy only.  It assumes the last safe
        # translational acceleration is realized by the low-level controller.
        # Replace this with external wrench reconstruction for paper experiments.
        return self.iebc.mass * (
            self._iebc_last_safe_acceleration_enu
            + np.array([0.0, 0.0, self.iebc.g], dtype=float)
        )
    ...
    def print_status(self):
        ...
        self.get_logger().info(
            ...
            f"IEBC: enabled={self.iebc.debug.get('enabled', False)} | active={self.iebc.debug.get('active', False)} | "
            f"P_I={self.iebc.debug.get('p_hat', 0.0):+.3f} W | Sbar={self.iebc.debug.get('s_bar', 0.0):.3f} J | "
            f"E_I={self.iebc.debug.get('e_i', 0.0):.3f}/{self.iebc.e_max:.3f} J | "
            f"h={self.iebc.debug.get('h_i', 0.0):+.3f} J | "
            f"aI={self.iebc.debug.get('a_nom_i', 0.0):+.2f}->{self.iebc.debug.get('a_safe_i', 0.0):+.2f} m/s^2\n"
            f"{'=' * 72}"
        )
